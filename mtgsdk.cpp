...
#define STR_BUFSIZE_LENG  (sizeof(stsendinfor) - kMaxBufferSize)
	...
	{
		printf("callback > conf_id: %d msg: %d uparam1: %d\n", conf_id, msg, uParam1);
		MtgSdk * pPlane = (MtgSdk *) user_data;
		if (!pPlane)
			return 0;
		switch (msg) {
...
bool MtgSdk::MtgInit() {
	...
	wx_mtg_init(NULL, NULL, 1024 + getpid(), 2048 + getpid());
	int http_port = 0;
	int udp_port = 0;
	int tcp_port = 443;
	...
	wx_mtg_set_setting(NULL, MTG_SET_OPT_HTTP_PORT, NULL, (void*)&http_port);
	wx_mtg_set_setting(NULL, MTG_SET_OPT_TCP_PORT, NULL, (void*)&tcp_port);
	wx_mtg_set_setting(NULL, MTG_SET_OPT_UDP_PORT, NULL, (void*)&udp_port);
...
bool MtgSdk::MtgUserJoin() 
{
	...
	int re = wx_mtg_register(m_conf_handle, const_cast<char *>(kVideoLivePathHead),NetVideoRecvData,this);
  
	//re = (*pfun_wx_mtg_register)(m_conf_handle, const_cast<char *>(kAudioLivePathHead),NetAudioRecvData,this);
	re = wx_mtg_register(m_conf_handle, const_cast<char *>(kAudioLivePathHead),NetAudioRecvData,this);
...
bool MtgSdk::MtgJoinConf(const char* pMtgIP, int confId,int uId) 
{
	...
	m_conf_handle = wx_mtg_join(const_cast<char *>(m_strMtgIP.c_str()), JoinCallback, this, 0, m_confId, m_userId, m_userSeq, "wxplanform", NULL);
...
int	MtgSdk::MtgSendVideoData(char * pdata,int length)
{
	...
	return wx_mtg_modify(m_conf_handle, 1, &update_item);
}
...
void MtgSdk::MtgAudioStream() 
{
	while (m_bHeartbeat) {
		...
		wx_mtg_detect_stream(m_conf_handle, MTG_DATA_TYPE_AUDIO);
		usleep(10000);
	}
}
